...
```sh
ibmcloud sat storage config create --name localvol-file-config --template-name local-volume-file --template-version 4.10 -p "label-key=storage" -p "label-value=localfile" -p "devicepath=/dev/xvde"
```
