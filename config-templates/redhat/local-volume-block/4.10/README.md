...
2. Login into the Cluster using `OC CLI`
    ...
    **Note** The target cluster version should be 4.10.X to use the local-volume-block version 4.10 template.
...
```sh
ibmcloud sat storage config create --name localvol-block-config --template-name local-volume-block --template-version 4.10 -p "label-key=storage" -p "label-value=localvol" -p "devicepath=/dev/xvdc"
```
