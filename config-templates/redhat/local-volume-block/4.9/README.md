...
2. Login into the Cluster using `OC CLI`
    ...
    **Note** The target cluster version should be 4.9.X to use the local-volume-block version 4.9 template.
...
```sh
ibmcloud sat storage config create --name localvol-block-config --template-name local-volume-block --template-version 4.9 -p "label-key=storage" -p "label-value=localvol" -p "devicepath=/dev/xvdc"
```
